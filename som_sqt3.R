...
som_grid <- somgrid(xdim =10 , ydim=10, topo="hexagonal",neighbourhood.fct="gaussian")
som_model = som(m, som_grid,rlen=100)

plot(som_model, type="changes")
plot(som_model, type="count", main="Node Counts",palette.name=coolBlueHotRed)

#carte  uniforme avec gaussian que avec bubbl
...
kn=kmeans(som_model$codes[[1]],4,nstart=300)
plot(som_model, type="codes",palette.name=coolBlueHotRed)
